...
#define TIME_TEXT_COLOR         0xffffffff //white

#define DUMP_MSGQ_KEY           1020
#define DUMP_MSGQ_MSG_TYPE      0x02

/* Sobel Edge Detection */
#define USE_SOBEL_EDGE_DETECTION    1

#if USE_SOBEL_EDGE_DETECTION
#define LCD_SCREEN_W            480
#define LCD_SCREEN_H            272
#define LCD_OUTPUT_IMG_SIZE (LCD_SCREEN_W*LCD_SCREEN_H*3/2)
...
#define DUMP_FILE_NAME          "image.nv12"
#endif  //USE_SOBEL_EDGE_DETECTION
...
/**
  ...
  * @param  inData: input image buffer (y data)
                 outData: output image buffer
                 computeWidth : input image width
                 computeWidth : output image width
                 inputPitch : input image pitch
                 outputPitch : output image pitch 
  * @retval none
  */
static void sobel_edge_detection
(
    uint8_t  *inData,
    uint8_t  *outData,
...
)
{
    ...
    int16_t gradY;
    uint32_t mag;
    uint32_t threshold = SOBEL_THRESHOLD;
    uint8_t roundShift = 3;
    ...
    for ( j = 1; j < computeHeight-2 ; j++)
    {
        for ( i = 1; i < computeWidth-2; i++)
        {
            gradX = 0;
            gradY = 0;
            ...
            for ( l = 0; l < 3; l++)
            {
                for ( k = 0; k < 3; k++)
                {
                    gradX+= coeffPtrX[k + l * 3] * inPtr[ k + l * inputPitch];
                    gradY+= coeffPtrY[k + l * 3] * inPtr[ k + l * inputPitch];
                }
            }

            gradX = (gradX + (1<< (roundShift - 1)) )>> roundShift;
            gradY = (gradY + (1<< (roundShift - 1)) )>> roundShift;
...
void * capture_thread(void *arg)
{
...
#if USE_SOBEL_EDGE_DETECTION
    uint8_t* pfbuf[4];
    uint8_t in_cambuf[VPE_OUTPUT_W*VPE_OUTPUT_H];
    uint8_t out_edgeImage[VPE_OUTPUT_IMG_SIZE];
    int uv_idx;
    struct timeval st;
    struct timeval et;
#endif
    ...
    while(1) {
        ...
        if(get_framebuf(capt, pfbuf) == 0) {
            for(uv_idx=0; uv_idx<VPE_OUTPUT_W*VPE_OUTPUT_H/2;) {
                pfbuf[1][uv_idx++] = MAKE_YUV_601_U(192, 192, 192);
                pfbuf[1][uv_idx++] = MAKE_YUV_601_V(192, 192, 192);
            }
            
            memcpy(in_cambuf, pfbuf[0], VPE_OUTPUT_W*VPE_OUTPUT_H); // y data
            
            /* Sobel Edge Detection */
            sobel_edge_detection(in_cambuf, out_edgeImage, VPE_OUTPUT_W, VPE_OUTPUT_H, VPE_OUTPUT_W, VPE_OUTPUT_W);
        
            memcpy(pfbuf[0], (char *)out_edgeImage, VPE_OUTPUT_W*VPE_OUTPUT_H); // y data
        }
        ...
        draw_operatingtime(vpe->disp, optime);
        ...
        if(data->dump_state == DUMP_READY) {
            if(get_framebuf(capt, pfbuf) == 0) {
                switch(capt->fourcc) {
                    case FOURCC('Y','U','Y','V'):
                        memcpy(data->dump_img_data, pfbuf[0], VPE_OUTPUT_IMG_SIZE);
                        break;
                    case FOURCC('N','V','1','2'):
                        memcpy(data->dump_img_data, pfbuf[0], VPE_OUTPUT_W*VPE_OUTPUT_H); // y data
                        memcpy(data->dump_img_data+VPE_OUTPUT_W*VPE_OUTPUT_H, pfbuf[1], VPE_OUTPUT_W*VPE_OUTPUT_H/2); // uv data
                        break;
                    default :
...
/**
  * @brief  Sobel edge the captured image dump and save to file
  * @param  arg: pointer to parameter of thr_data
  * @retval none
  */
...
void signal_handler(int sig)
{
    if(sig == SIGINT) {
        ...
        printf("-- 4_camera_vpe_edgedect_disp example End --\n");
    }
}
...
int main(int argc, char **argv)
{
    ...
    printf("-- 4_camera_vpe_edgedect_disp example Start --\n");
